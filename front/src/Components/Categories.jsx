...
import { FiEdit2, FiTrash2, FiX, FiFolder, FiImage, FiUpload } from "react-icons/fi";
import { FaSpinner, FaPlus, FaFolder } from "react-icons/fa";
...
function Categories() {
    ...
    if (button === "{bksp}") {
      ...
      if (inputName) {
        if (isEditing) {
          ...
          setNewCategory(prev => ({ ...prev, [inputName]: newValue }));
        }
      }
  ...
  }, [inputName, isEditing, editData, newCategory, keyboardInput]);
  ...
  // Fetch catégories avec AxiosClient et gestion d'erreur
  ...
  // Ajouter une catégorie avec AxiosClient et gestion d'erreur
  const addCategory = async () => {
    try {
      ...
      console.error("Erreur add catégorie:", error);
  ...
  // Modifier une catégorie avec AxiosClient et gestion d'erreur
  const updateCategory = async () => {
    try {
      ...
      const response = await AxiosClient.post(`/categories/${editData.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
  ...
  // Supprimer une catégorie avec AxiosClient et gestion d'erreur
  ...
  const handleInputFocus = (field) => {
    ...
    const currentValue = isEditing ? editData[field] || "" : newCategory[field] || "";
          ...
          <div className="overflow-x-auto max-h-[80vh] overflow-y-auto">
                    ...
                    <td colSpan="4" className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center">
                        <div className="w-16 h-16 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-2xl flex items-center justify-center mb-4">
                          <FaFolder className="w-8 h-8 text-indigo-400" />
                        </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          {category.image_url ? (
                            ...
                            <div className="flex-shrink-0 h-12 w-12 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg flex items-center justify-center border border-indigo-200 shadow-sm">
            ...
            <div className="sm:hidden">
              {loading ? (
                ...
                // Message quand il n'y a pas de catégories (mobile)
                <div className="text-center py-16 px-4">
                  <div className="flex justify-center mb-4">
                    <div className="w-20 h-20 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-2xl flex items-center justify-center">
                      <FaFolder className="w-10 h-10 text-indigo-400" />
                    </div>
                  </div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">Aucune catégorie trouvée</h3>
                  <p className="text-gray-500 mb-6">Commencez par ajouter votre première catégorie.</p>
                ...
                // Afficher les catégories (mobile)
                <div className="space-y-4 p-0">
                  {categories.map((category) => (
                    <div key={category.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition duration-150">
                      <div className="flex items-center space-x-4 mb-4">
                        {category.image_url ? (
                          ...
                          <div className="flex-shrink-0 h-16 w-16 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg flex items-center justify-center border border-indigo-200 shadow-sm">
            ...
            <div ref={formRef} className="bg-white rounded-xl sm:rounded-2xl shadow-xl w-full max-w-2xl mx-2 transform transition-all max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        type="text"
                        value={isEditing ? editData.name : newCategory.name}
                ...
                <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 pt-6 border-t border-gray-200">
                  ...
                  <button
                    type="submit"
                    disabled={loading || (!isEditing && !newCategory.name) || (isEditing && !editData.name)}
